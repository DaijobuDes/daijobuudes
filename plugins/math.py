...
class Math(commands.Cog):
    ...
    async def dice(self, ctx, times = 1, sides = 6):
        if times == 0 and sides == 0:
            await ctx.send(f'You can\'t roll 0')
    ...
    async def pow(self, ctx, base : int, exponent : int):
        try: # Not using math.pow() since it is vulnerable to not responding on calculating large values
            answer = ldexp(base, exponent)
        except OverflowError:
            await ctx.send(f'Nuh uh, nice try. Math range error')
        except ValueError:
